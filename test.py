...
framework.add_signal_formulae(
    ...
    &
    Signal(ModelSignal.CROSS_ENTROPY_CONFIDENCE, is_model_signal=True, extra_args={'conf_threshold': 0.8}))
    |
    Signal(ModelSignal.CROSS_ENTROPY_CONFIDENCE, is_model_signal=True, extra_args={'conf_threshold': 0.4})
)
...
print("Selected %d from a total of %d data-points" % (framework.selected_count, framework.predicted_count))